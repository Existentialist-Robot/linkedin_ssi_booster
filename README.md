...
| `--type post`        | `--curate`                                               | Schedule curated posts **directly** to the next available Buffer queue slot. LinkedIn: clean post body (no hashtags, no link) + first comment containing hashtags and source URL. X and Bluesky: 3-post thread (hook / insight / close). |
...
**LinkedIn post body vs. first comment:** When using `--type post --channel linkedin`, the post body contains only the commentary text. Hashtags and the source URL are placed exclusively in the first comment — this is intentional. LinkedIn's algorithm down-ranks posts with outbound links in the body, and hashtags in the first comment perform just as well for discoverability without cluttering the post.
