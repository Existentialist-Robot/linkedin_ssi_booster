...
| `tests/test_continual_learning.py`                                                                                                                                                                                                                                                                                                                                                       | ExtractedFact/ExtractedKnowledgeGraph schema, loader, normalization, deduplication, integration, and **comprehensive noise filtering pipeline** — 88 parametrized filter regression tests covering every guard in `_extraction.py`: adversative conjunctions, conditional tutorial fragments, anthropomorphism, first-person narration, RSS boilerplate, rhetorical questions, dangling-pronoun openers, heading+pronoun concatenations, quantity references, "rarely something" advisory, truncated fragments, URL sentences, pipe navigation, HuggingFace/GitHub blobs, podcast preambles, scene-setters, survey openers, educational preambles, self-promotion, camelCase mangled joins, table blobs, version-list blobs, generic fillers, and bare availability announcements. Includes edge-case exemption tests (availability + "enabling", filler + metric, camelCase in product name + year, unique version numbers). |
...
| If` newline splits), community tag detection (`AI/GovTech/Ottawa`slash-separated),`TruthGateMeta`field coverage, and **multi-project sentence false-positive guard** (keyword legitimately owned by one project in the sentence is not falsely attributed to another co-mentioned project, e.g.`Answer42`'s Spring Batch not flagged because `LinkedIn SSI Booster` is also referenced). |
